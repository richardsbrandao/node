class CommentService {
    ...
    async findAll() {
        ...
        console.log(CommentService.repository)
        return CommentService.repository;
    }

    save(comment) {
        CommentService.repository = CommentService.repository.map((savedComment) => {
            if(savedComment.id == comment.id) {
                savedComment.email = comment.email;
                savedComment.body = comment.text;
            }
            return savedComment
        })
        console.log(CommentService.repository)
    ...
    remove(id) {
        CommentService.repository = CommentService.repository.filter((savedComment) => id != savedComment.id)
    }
}
